...
def parse(fileno):
    ...
    service_time = (float(service_time) - float(start_time))*1000

    reading.append(str(parse_time))
    reading.append(str(routing_time))
    reading.append(str(service_time))
...
def main():
    ...
    with open(measurement_output, "w") as f:        
            writer = csv.writer(f, delimiter=',')
            writer.writerow(["Start Timestamp", "Parse Config Timestamp", "Routing Ready Timestamp","Service Discovered Timestamp", "Parse Config time [ms]", "Routing ready time [ms]", "Service Discovery time [ms]"])
